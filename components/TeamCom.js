import React from 'react'
import { useState } from 'react';
import { FaArrowCircleDown,FaArrowCircleUp } from 'react-icons/fa';
import Image from 'next/image'

const TeamCom = ({Id,obj}) => {
    const [Index, setIndex] = useState(2020);
    const handleSetIndex = (Id) => {
        if(Index !== Id){
            setIndex(Id);
        }else{
            setIndex(false);
        }
    }
  return (
    <>
    <div onClick={()=>handleSetIndex(Id)} className='container justify-center mx-auto'>
                <div className='justify-start px-5 pt-6 lg:px-28 '>
                    <div  className='hover:divide-x cursor-pointer justify-center items-center text-start py-4 px-6 rounded-lg text-3xl font-medium italic border bg-slate-900 text-stone-100'>{Index !== Id ?(<FaArrowCircleDown className='inline-block text-4xl'/>):(<FaArrowCircleUp className='inline-block text-4xl'/>)} <span className='inline-block px-4'>  Batch of {Id}</span></div>
                </div>
                <div className='scroll-smooth'>
                    <section className="text-gray-600 body-font scroll-smooth">
                        <div className="container px-5 lg:px-32  pt-1 mx-auto">
                            {Index === Id && <div className="flex flex-wrap  justify-center">
                            {/* const result = words.filter(word => word===2020); */}
                            {obj.map((item)=>{
                                    return <div key={item._id} className="p-2 lg:w-1/4">
                                        <div className="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
                                            <div className='transition ease-in-out delay-150 cursor-pointer  hover:-translate-y-2 hover:scale-110 duration-500'>
                                                <Image alt='humanImage'  src={item.image.asset.url} className='object-fill aspect-auto' width={500} height={650}></Image>
                                            </div>
                                            <div className="p-6">
                                                <h2 className="tracking-widest text-xs title-font font-medium text-gray-400 mb-1">CATEGORY</h2>
                                                <h1 className="title-font text-lg font-medium text-gray-900 mb-3">{item.name}</h1>
                                                <p className="leading-relaxed text-left  mb-3">{item.designation}</p>
                                                <p className="leading-relaxed text-left  mb-3">{item.location}</p>
                                                <div className="flex items-center flex-wrap ">
                                                    <a className="text-indigo-500 inline-flex items-center md:mb-2 lg:mb-0">Learn More
                                                        <svg className="w-4 h-4 ml-2" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round">
                                                            <path d="M5 12h14"></path>
                                                            <path d="M12 5l7 7-7 7"></path>
                                                        </svg>
                                                    </a>
                                                    <span className="text-gray-400 mr-3 inline-flex items-center lg:ml-auto md:ml-0 ml-auto leading-none text-sm pr-3 py-1 border-r-2 border-gray-200">
                                                        <svg className="w-4 h-4 mr-1" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                                                            <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                                                            <circle cx="12" cy="12" r="3"></circle>
                                                        </svg>1.2K
                                                    </span>
                                                    <span className="text-gray-400 inline-flex items-center leading-none text-sm">
                                                        <svg className="w-4 h-4 mr-1" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                                                            <path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z"></path>
                                                        </svg>6
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    })
                                    }
                            </div>}
                        </div>
                    </section>
                </div>
            </div>
    </>
    
  )
}

export default TeamCom